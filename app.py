...
def show():
    ...
    inputimage = Image.open(pic)
    inputimage = inputimage.resize((28, 28))
    print(inputimage)
    image = np.array(inputimage).reshape(-1, 28, 28, 3)
    print("target:",image.data)
    result = SCD.model.predict(image)

    result = result.tolist()
    print(result)
    max_pro = max(result[0])
    class_index = result[0].index(max_pro)
    print(class_index)
    result = SCD.classes[class_index]
